...
export function useUrlState<T>(
  ...
  // Create URLSearchParams from React Router's location.search
          ...
          try {
            ...
            if (parsed && typeof parsed === "object") {
              ...
              if (key === "dateRange") {
                ...
                // Only warn if the properties are missing, not if they're empty strings
                if (dateRange.from_date === undefined || dateRange.to_date === undefined) {
  ...
  const updateUrlNow = useCallback(
    (params: URLSearchParams) => {
      ...
      // Update the URL immediately using React Router
