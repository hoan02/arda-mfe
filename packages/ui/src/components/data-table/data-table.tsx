...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  // Ensure dateRange is always properly initialized
  const safeDateRange = useMemo(() => {
    if (!dateRange || typeof dateRange !== "object") {
      console.warn("dateRange is invalid, using default:", dateRange);
  ...
  // Safe wrapper for setDateRange to prevent invalid values
  const safeSetDateRange = useCallback(
    (
      value:
      ...
      const resolvedValue =
        typeof value === "function" ? value(safeDateRange) : value;

      // Validate the resolved value
      if (!resolvedValue || typeof resolvedValue !== "object") {
        console.warn("Invalid dateRange value:", resolvedValue);
        setDateRange({ from_date: "", to_date: "" });
        return;
      }

      setDateRange({
        from_date: resolvedValue.from_date || "",
        to_date: resolvedValue.to_date || "",
      });
    },
      ...
      const fetchData = async () => {
        try {
          ...
          const result = await (
            ...
            from_date: safeDateRange.from_date,
            to_date: safeDateRange.to_date,
                ...
                >
                  {Array.from({ length: columns.length }).map((_, j, array) => (
                    <TableCell
                      ...
                      className="py-2 truncate max-w-0 text-left"
                      tabIndex={-1}
                    >
